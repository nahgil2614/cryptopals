...
def InvCiphers( mes, w ):
    plain = ''
    
    for i in range(0, len(mes), 16):
        plain += InvCipher( mes[i:i+16], w )
...
def main():
    ...
    plain = InvCiphers(message, w)
    print(plain)

    cipher = Ciphers(string_to_array(plain), w)
    print(cipher)

    plain = InvCiphers(string_to_array(cipher), w)
    print(plain)
